...
export const useWorkflowStore = create<WorkflowState>((set, get) => ({
  ...
  generateStagePrompt: () => {
    ...
    const stage = workflow.stages.find((s) => s.id === activeStageId)
    if (!stage) return ''
    return generateStagePrompt(stage.blocks, stage.customText, workflow.characterContext)
  },
  ...
  generateAllPrompts: () => {
    ...
    return workflow.stages.map((stage) => {
      ...
      return {
        stageId: stage.id,
        label: meta.label,
        prompt: generateStagePrompt(stage.blocks, stage.customText, workflow.characterContext),
